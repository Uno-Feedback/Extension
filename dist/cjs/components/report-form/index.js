...
const handleSubmit = (acceptButton, onSubmit) => {
    ...
    observable_1.default.subscribe("enableButton", () => enableButton(acceptButton));
};
const createFooter = ({}, onSubmit) => {
    ...
    submitButtonText.innerText = langs_1.lang.en.reportForm.submit;
    submitButtonIcon.innerHTML = svg_1.submitIcon;
    ...
    createRadio(sendToWrapper, buttonGroup, sendOptions, 0, langs_1.lang.en.reportForm.sendTo.label, "sendTo", false);
    // - Append the 'Send To' to Footer
    footer.appendChild(sendToWrapper);
    // - Append Submit Button to Footer
    footer.appendChild(submitButton);
};
const createInput = (row, col, inputLabel, label, name, initialValue, hasPlaceholder, isRequired) => {
    // Input
    ...
    span.innerText = `${label} ${langs_1.lang.en.reportForm.errorMessage}`;
...
const createSelect = (row, col, selectLabel, options, label, name, active) => {
    // Select
...
const createRadio = (row, col, options, active, label, name, isRequired) => {
    // Create label
    const span = document.createElement("span");
...
const createTextArea = (row, col, textAreaLabel, label, name, hasPlaceholder, isRequired) => {
...
const createRadioWrapper = (row, col) => {
    ...
    ///// Report: 2
    const typeOptions = [
        { label: langs_1.lang.en.reportForm.type.bug, value: "1", color: "#F04438" },
        { label: langs_1.lang.en.reportForm.type.report, value: "2", color: "#F79009" },
        { label: langs_1.lang.en.reportForm.type.feature, value: "3", color: "#17B26A" }
    ];
    createRadio(buttonGroup, innerCol.cloneNode(true), typeOptions, 0, langs_1.lang.en.reportForm.type.label, "type", true);
    // Priority
    //// highest: 1
    //// high: 2
    //// medium: 3
    ...
    const priorityOptions = [
        { label: langs_1.lang.en.reportForm.priority.low, value: "4", color: "#F79008" },
        { label: langs_1.lang.en.reportForm.priority.medium, value: "3", color: "#2A70FE" },
        { label: langs_1.lang.en.reportForm.priority.high, value: "2", color: "#E14EB6" }
    ];
    createSelect(buttonGroup, innerCol.cloneNode(true), inputLabel.cloneNode(true), priorityOptions, langs_1.lang.en.reportForm.priority.label, "priority", 1);
    container.appendChild(buttonGroup);
    col.appendChild(container);
    // Col append to buttonGroup
    row.appendChild(col);
};
const createForm = ({ fileName, fileSize }) => {
    // Wrapper
    ...
    // Subject
    createInput(formRow, col.cloneNode(true), inputLabel.cloneNode(true), langs_1.lang.en.reportForm.subject, "subject", "", false, true);
    formRow.appendChild(divider.cloneNode(true));
    // Description
    createTextArea(formRow, col.cloneNode(true), inputLabel.cloneNode(true), langs_1.lang.en.reportForm.description, "description", false, true);
    formRow.appendChild(divider.cloneNode(true));
    ...
    fileTitle.innerText = langs_1.lang.en.reportForm.fileTitle;
...
const createInfo = () => {
    ...
    const tabs = [
        { title: langs_1.lang.en.reportForm.info.info },
        { title: langs_1.lang.en.reportForm.info.network },
        { title: langs_1.lang.en.reportForm.info.console }
    ];
    ...
    const information = [
        {
            label: `${langs_1.lang.en.reportForm.info.url}:`,
            data: window.location.href
        },
        {
            label: `${langs_1.lang.en.reportForm.info.captured}:`,
            data: createName()
        },
        {
            label: `${langs_1.lang.en.reportForm.info.deviceInfo}:`,
            data: navigator.userAgent
        },
        {
            label: `${langs_1.lang.en.reportForm.info.OS}:`,
            data: OS()
        },
        {
            label: `${langs_1.lang.en.reportForm.info.windowSize}:`,
...
const createContent = ({ fileSize, fileName }) => {
...
const initialInnerElements = ({}, { fileSize, fileName }, onSubmit) => {
 ...
 * ////////////////////
 * **/
const appendFormToModal = ({ fullName, email, avatar }, { fileSize, fileName }, onSubmit) => {
    (0, modal_1.default)((0, modal_1.createTitle)(langs_1.lang.en.reportForm.title), () => destroyRequestForm()).then(modalContent => {
...
const openReportFormModal = (recordedBlob) => {
    const fileSize = formatBytes(recordedBlob.size, 2);
    const { fullName, email, avatar } = states_1.default.user;
    appendFormToModal({ fullName, email, avatar }, { fileSize, fileName: createName() }, () => {
        /* todo set loading true */
        (0, request_1.default)(recordedBlob, createName(), storeValues)
            .then(response => {
...
exports.default = openReportFormModal;
