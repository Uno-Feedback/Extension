...
const report_form_1 = require("../report-form");
...
const appendVideoToModal = (fileName, fileSize) => {
    (0, modal_1.default)((0, modal_1.createTitle)(`${langs_1.lang.en.videoPreview.title} <small>(${fileName} - ${fileSize})</small>`), () => { }).then(modalContent => {
...
const goToNextStep = (recordedBlob) => {
    destroyVideoElements().then(() => {
        (0, report_form_1.default)(recordedBlob);
    });
};
const toggleMute = () => {
    if (isMute) {
        volume.innerHTML = svg_1.volumeIcon;
        isMute = false;
        videoElement.muted = false;
        return;
    }
    volume.innerHTML = svg_1.volumeOffIcon;
    videoElement.muted = true;
    isMute = true;
};
const togglePlay = () => {
    if (isPlaying) {
        playPauseButton.innerHTML = svg_1.playIcon;
        isPlaying = false;
        videoElement.pause();
        return;
    }
    playPauseButton.innerHTML = svg_1.pauseIcon;
...
const initialInnerElements = (recordedBlob) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    // SVG Poster
    const svg = new DOMParser().parseFromString(svg_1.videoPoster, "text/xml");
    ...
    videoTimeline35.innerHTML = svg_1.timeline35;
    videoTimeline60.innerHTML = svg_1.timeline60;
    ...
    // brush
    brush.innerHTML = svg_1.brushIcon;
    ...
    // fullscreen
    fullScreenButton.innerHTML = svg_1.fullscreenIcon;
    ...
    confirmationButton.innerHTML = `${langs_1.lang.en.videoPreview.confirm}<span>${svg_1.confirmIcon}</span>`;
